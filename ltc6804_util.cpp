 ...
 */
void ltc6804util_encodeCommand(uint8_t addr,ltc6804_command_codes_e cmd, uint8_t *b) {
    
    if (b == NULL) {
        return;
    }
  uint16_t cmd_ = static_cast<uint16_t> (cmd);
    cmd_ &= 0x7ff;                               // mask unused bits in cmd
    addr &= 0x0F;                               // mask unused bits in addr
    uint8_t cmdHi = (uint8_t)(cmd_ >> 8);      // CMD hi byte
    uint8_t cmdLo = (uint8_t)cmd_;             // CMD lo byte
...
void ltc6804util_decodeStatA(void *g, uint8_t *b) {
    ...
    struct ltc6804_stat_reg_a_s *r = static_cast<struct ltc6804_stat_reg_a_s*> (g);
    // test PEC
...
void ltc6804util_decodeStatB(void *g, uint8_t *b) {
    ...
    struct ltc6804_stat_reg_b_s *r = (struct ltc6804_stat_reg_b_s*) (g);
    // test PEC
    if (checkGroupPec(b) == PEC_ERROR) {
        r->pecOk = PEC_ERROR;
    }
    ...
    for (byte = 0; byte < 3; byte++) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            *currFlag = (enum ltc6804_cell_flags_e) ((b[2 + byte] >> (i * 2)) & 0x03);
            currFlag++;
        }
    }
    r->rev = b[5] >> 4;
...
void ltc6804util_decodeCfg(void *g, uint8_t *b) {
    ...
    struct ltc6804_cfg_reg_s *r =(struct ltc6804_cfg_reg_s *) g;
    // test PEC
    if (checkGroupPec(b) == PEC_ERROR) {
        r->pecOk = PEC_ERROR;
    }
    ...
    r->dcto = (ltc6804_DCTO_e) (b[5] >> 4);

    //Serial.print("CONFIG at decode: GPIO %l  refon %l  swtrd %l  adcopt %l  vuv %l  vov %l  dcc %l,  dctc %l",r->gpio,r->refon,r->swtrd,r->adcopt,r->vuv,r->vov,r->dcc,r->dcto);
...
void ltc6804util_encodeCfg(uint8_t *b, void *g) {
    ...
    //Logger::debug("CONFIG: b0 %l  b1 %l  swtrd %l  adcopt %l  vuv %l  vov %l  dcc %l,  dctc %l",cfg.gpio,cfg.refon,cfg.swtrd,cfg.adcopt,cfg.vuv,cfg.vov,cfg.dcc,cfg.dcto);
...
void ltc6804util_decodeCV(void *g, uint8_t *b) {
    ...
    struct ltc6804_cell_voltage_reg_s *r = (struct ltc6804_cell_voltage_reg_s *) g;
    // test PEC
    if (checkGroupPec(b) == PEC_ERROR) {
        r->pecOk = PEC_ERROR;
    }
...
void ltc6804util_decodeAuxA(void *g, uint8_t *b) {
    ...
    struct ltc6804_aux_reg_a_s *r =(struct ltc6804_aux_reg_a_s *) g;
    // test PEC
    if (checkGroupPec(b) == PEC_ERROR) {
        r->pecOk = PEC_ERROR;
    }
...
void ltc6804util_decodeAuxB(void *g, uint8_t *b) {
    ...
    struct ltc6804_aux_reg_b_s *r =(struct ltc6804_aux_reg_b_s *) g;
    // test PEC
    if (checkGroupPec(b) == PEC_ERROR) {
        r->pecOk = PEC_ERROR;
    }
